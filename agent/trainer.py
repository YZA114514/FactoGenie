...
from environment.factory_environment import FactoryEnv
...
def train(params):
    ...
    net = model_cls(env.observation_space.shape, env.action_space.n).to(device)
    tgt_net = model_cls(env.observation_space.shape, env.action_space.n).to(device)
